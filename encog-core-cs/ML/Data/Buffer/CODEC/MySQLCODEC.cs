...
namespace Encog.ML.Data.Buffer.CODEC
{
    ...
    /// </summary>
    public class MySQLCODEC : IDataSetCODEC
    {
        /// <summary>
        ...
        /// </summary>
        private readonly MySqlCommand _statement;

        /// <summary>
        /// Holds results from the SQL query.
        /// </summary>
        private MySqlDataReader _results;
        ...
        public MySQLCODEC(String sql, int inputSize,
                        int idealSize, String connectString)
        {
            ...
            _statement = new MySqlCommand(sql, _connection);
            _statement.Prepare();
        }
        ...
        public bool Read(double[] input, double[] ideal, ref double significance)
        {
            ...
            {
                input[i] = _results.GetDouble(i);
            }

            if (_idealSize > 0)
            {
                for (int i = 0; i < _idealSize; i++)
                {
                    ideal[i] = _results.GetDouble(i + _inputSize);
                }
            }
