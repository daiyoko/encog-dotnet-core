...
{
    [TestClass]
    public class TestOandaLoader
    {
        [TestMethod]
        public void Loader()
        {
            IMarketLoader loader = new OandaFinanceLoader();
            var from = new DateTime(2014, 12, 10);
            var to = new DateTime(2014, 12, 11);
            ICollection<LoadedMarketData> list = loader.Load(new TickerSymbol("JPY/HKD"), null, from, to);
